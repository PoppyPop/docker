...
type Remote interface {
	...
	Hosts() []string
	NewCert(jsonData []byte) (*JsonCertResponse, error)
}
...
func (srv *server) postRaw(url string, jsonData []byte) ([]byte, *api.Response, error) {
	...
	if err != nil {
		...
		return nil, nil, errors.Wrap(errors.APIClientError, errors.ClientHTTPError, err)
	}
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		return nil, nil, errors.Wrap(errors.APIClientError, errors.IOError, err)
	}
	resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		log.Errorf("http error with %s", url)
		return nil, nil, errors.Wrap(errors.APIClientError, errors.ClientHTTPError, stderr.New(string(body)))
	}
	...
	if !response.Success || response.Result == nil {
		if len(response.Errors) > 0 {
			return nil, nil, errors.Wrap(errors.APIClientError, errors.ServerRequestFailed, stderr.New(response.Errors[0].Message))
		}
		return nil, nil, errors.New(errors.APIClientError, errors.ServerRequestFailed)
	}
...
// post connects to the remote server and returns a Response struct
func (srv *server) post(url string, jsonData []byte) (res *api.Response, err error) {
	_, res, err = srv.postRaw(url, jsonData)
	return
}
...
type jsonResponse struct {
	Result         JsonCertResponse `json:"result"`
}

// Sign sends a newcert request to the remote CFSSL server,
// receiving a signed certificate or an error in response.
// It takes the serialized JSON request to send.
func (srv *server) NewCert(jsonData []byte) (res *JsonCertResponse, err error) {

	url := srv.getURL("newcert")
	body, _, err := srv.postRaw(url, jsonData)
